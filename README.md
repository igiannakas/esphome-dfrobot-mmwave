...
- **Both C4001 modes.** The SEN0609 and SEN0610 can run as a presence detector or as a speed-and-distance tracker that reports how far away the target is and how fast it moves. You can switch between them from Home Assistant.
...
There's a complete, commented production config for a XIAO ESP32-C6 in `examples/sen0609-xiao-esp32c6.yaml`, and bench configs for all three radars in `examples/bench/`.
...
**The C4001's operates in two distinct modes.** Switching `work_mode` restarts the radar into the other mode (firmware). Each mode keeps its own range and LED setting; the presence-only settings (sensitivities, latencies, inhibit time, reporting) are unavailable in speed mode and the speed-only ones (`speed_micro_motion`, `speed_threshold_factor`, the target sensors) are unavailable in presence mode. Speed mode has no delays at all: the OUT pin and the reports follow the target instantly, and the target sensors update about ten times a second, so it is recommended to put a `throttle_with_priority` filter on them if you enable them. Plain `throttle` can swallow the single "no target" update and leave the last distance on screen.
...
- **target_timeout** (default 2 s): how long without a target report before the target sensors are cleared. Only matters for the SEN0395's target list and the C4001's speed mode.

### Entities

Add any of these under their respective platform. Everything is optional and takes the usual ESPHome entity options (`name`, `id`, `icon`, `filters` and so on). "Presence" and "speed" in the mode column say which C4001 work mode the entity belongs to; the SEN0395 only has the one mode.
...
| `max_range` | both | 0.15–9.45 m | 2.4–26 m | 2.4–12 m | 26 m is the speed mode's limit; presence mode goes to 25 |
| `trigger_range` | presence | — | 2.4–25 m | 2.4–12 m | distance within which a new target triggers detection; never above `max_range` |
| `sensitivity` | both | 0–9 | — | — | |
| `hold_sensitivity` | presence | — | 0–9 | 0–9 | how easily a target is kept |
| `trigger_sensitivity` | presence | — | 0–9 | 0–9 | how easily one is acquired (DFRobot suggest 2–6) |
| `on_latency` | presence | 0–100 s | 0–2 s | 0–2 s | how long a target must be seen before "occupied" |
| `off_latency` | presence | 0.5–1500 s | 2–1500 s | 2–1500 s | how long after the last detection before "clear" |
| `inhibit_time` | presence | — | 0.3–60 s | 0.3–60 s | dead time after each change of the OUT pin |
| `uart_report_period` | presence | 0.025–1500 s | 0.2–1500 s | 0.2–1500 s | keep-alive interval of the serial reports |
| `speed_threshold_factor` | speed | — | 0–65535 | 0–65535 | |
...
| `speed_micro_motion` | SEN0609, SEN0610 (speed) | |
...
| Key | Models | What it is |
|---|---|---|
| `target_count` | all | targets seen (SEN0395), or 0/1 in C4001 speed mode |
| `target_1_distance` … `target_8_distance` | SEN0395 (1–8), C4001 (1, speed mode) | metres |
| `target_1_snr` … `target_8_snr` | SEN0395 | |
| `target_1_speed` | SEN0609, SEN0610 (speed) | m/s, positive moving away, negative approaching |
| `target_1_energy` | SEN0609, SEN0610 (speed) | the radar's own strength figure, no unit |

**text_sensor**

| Key | What it is |
|---|---|
| `status` | `probing`, `reading`, `applying`, `running`, `stopped`, `link_lost` or `unsupported_firmware` |
...
4. **Check whether a factory reset on a C4001 also resets the speed mode's settings.** It restores the presence mode's; the other mode wasn't checked.
...
| `examples/` | a production config and the bench configs |
